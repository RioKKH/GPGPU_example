...
int main(void)
{
	...
	else
	{
        ...
        cudaHostAlloc((void**)&BGR_src, sizeof(BGR) * bmp.width * bmp.height, cudaHostAllocMapped);
        cudaHostAlloc((void**)&BGR_dst, sizeof(BGR) * bmp.width * bmp.height, cudaHostAllocMapped);
        memcpy(BGR_src, bmp.pixel, sizeof(BGR) * bmp.width * bmp.height);

		negativeBGR<<<block, thread>>>(BGR_src, bmp.width, bmp.height, BGR_dst);
		...
		// dev_bmp = swap;
		// 画像をホストメモリへコピー
        memcpy(bmp.pixel, BGR_dst, sizeof(BGR) * bmp.width * bmp.height);
		// cudaMemcpy(bmp.pixel, dev_bmp, sizeof(BGR) * bmp.width * bmp.height, cudaMemcpyDeviceToHost);
