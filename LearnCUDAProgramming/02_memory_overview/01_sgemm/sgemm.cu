...
#define BLOCK_DIM_Y 16
    ...
    //! Compute reference adta set matrix multiply on GPU
    //! C = alpha * A * B + beta * c
        ...
        const float *A, const float *B, float *C, int N, int M, int K, float alpha, float beta)
{
    ...
    // initial start on operation as a warm start
    ...
    // Waits for GPU operation finish and recorded the time
    sdkStopTimer(&timer);
...
int main()
{
    ...
    // initialize randomized vlaues for memory space
    random_init(A, N * K);
    random_init(B, K * M);
    random_init(C, N * M);
